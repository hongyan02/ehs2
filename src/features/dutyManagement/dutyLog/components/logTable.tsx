...
const column = createColumnHelper<DutyLogData>();

export const dutyLogColumns = (
  onView: (log: DutyLogData) => void,
  onDelete: (id: number) => void
): ColumnDef<DutyLogData, any>[] => [
...
export default function LogTable({ data, onView, onDelete }: LogTableProps) {
  const table = useReactTable({
    data,
    columns: dutyLogColumns(onView, onDelete),
      ...
      <table className="w-full border-collapse text-sm">
        ...
        <tbody className="[&_tr:last-child]:border-0">
          {table.getRowModel().rows.length > 0 ? (
            ...
            <tr>
              <td colSpan={dutyLogColumns(onView, onDelete).length} className="h-24 text-center">
                暂无数据
              </td>
            </tr>
          )}
