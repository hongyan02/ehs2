...
export default function PointSignboardPage() {
    ...
    // Footer Period (Start/End of Last Month)
    ...
    return (
        <div className="h-screen w-full bg-slate-50 text-slate-900 font-sans px-4 pt-4 pb-2 overflow-hidden flex flex-col relative transition-opacity duration-700 ease-in-out" style={{ opacity: mounted ? 1 : 0 }}>
            ...
            <div className="absolute top-0 left-0 w-full h-80 bg-gradient-to-b from-red-600 to-slate-50 opacity-10 pointer-events-none" />
            <div className="absolute -top-20 -right-20 w-80 h-80 bg-red-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse" />
            <div className="absolute -top-20 -left-20 w-80 h-80 bg-orange-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse" style={{ animationDelay: "2s" }} />
            ...
            <main className="relative z-10 flex-1 flex flex-col gap-2 overflow-hidden pt-10">
                ...
                <div className="bg-white/70 backdrop-blur-xl rounded-xl shadow-xl border border-white/50 flex-1 overflow-hidden flex flex-col min-h-0">
                    <div className="grid grid-cols-12 gap-2 px-4 py-2 border-b border-slate-100 font-semibold text-slate-500 text-xs uppercase tracking-wider shrink-0 bg-white/50">
                        ...
                        <div className="col-span-1 text-center">工号</div>
                        <div className="col-span-1 text-center">当月积分</div>
                        <div className="col-span-1 text-center">KPI</div>
                        <div className="col-span-1 text-center">总积分</div>
                        <div className="col-span-1"></div>
                        <div className="col-span-5 text-left">积分项 (Top 3)</div>
                    </div>
            ...
            <footer className="relative z-10 shrink-0 py-1 text-center text-[10px] text-slate-400 border-t border-slate-100/50 mt-1">
                统计周期：{startDate.toLocaleDateString('zh-CN')} ~ {endDate.toLocaleDateString('zh-CN')}
            </footer>
        </div>
    );
}
    ...
    return (
        <div
            className={cn(
                ...
                bgColor,
                isCenter ? "h-72 justify-center z-10 scale-102 border-2 shadow-lg" : "h-60 justify-end"
            )}
        >
            ...
            <div className="flex flex-col gap-0.5 w-full px-1 my-1 flex-1 overflow-hidden justify-center">
                {item.pointItems?.slice(0, 3).map((pi: string, idx: number) => (
                    <div key={idx} className="text-[10px] text-slate-600 bg-white/40 px-1 rounded truncate w-full text-center">
                        {pi}
                    </div>
                ))}
            </div>

            <div className="grid grid-cols-2 gap-1.5 w-full mt-auto shrink-0">
            ...
            <div className="mt-1.5 w-full bg-gradient-to-r from-red-500 to-orange-500 text-white py-1 rounded-lg shadow-sm shrink-0">
...
function ListItem({ item, rank }: { item: any; rank: number }) {
    return (
        <div
            className="grid grid-cols-12 gap-2 px-2 py-0.5 border-b border-slate-100 last:border-0 items-center hover:bg-white/60 transition-colors"
        >
            <div className="col-span-1 text-center font-bold text-slate-400 text-xs text-slate-500">#{rank}</div>
            <div className="col-span-1 text-center text-sm font-bold text-slate-700">{item.name}</div>
            <div className="col-span-1 text-center text-xs text-slate-400">{item.no}</div>
            <div className="col-span-1 text-center font-mono font-medium text-blue-600 text-sm">{item.totalPoints}</div>
            <div className="col-span-1 text-center font-mono font-medium text-green-600 text-sm">{item.kpiValue.toFixed(1).replace(/\.0$/, "")}</div>
            <div className="col-span-1 text-center">
                <span className="font-bold text-base text-slate-800">{item.finalTotal.toFixed(1).replace(/\.0$/, "")}</span>
            </div>
            <div className="col-span-1"></div>
            <div className="col-span-5 text-[10px] text-slate-500 flex flex-nowrap overflow-hidden gap-1 items-center h-full justify-start">
